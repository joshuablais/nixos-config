...
(defun jb/get-cliphist-entries ()
  "Get clipboard history from cliphist, newest first."
  (when (executable-find "cliphist")
    (let ((output (shell-command-to-string "cliphist list")))
      ;; cliphist list already returns newest first, so we do NOT reverse
      (split-string output "\n" t)))) 
...
(defun jb/clipboard-manager ()
  ...
  (let* ((cliphist-items (jb/get-cliphist-entries))
         ...
         ;; Combine: newest cliphist items first, then newest kill-ring items
         ;; delete-dups ensures no duplicates and preserves order.
         (all-items (delete-dups (append cliphist-items kill-ring-items)))
         ;; Create (display-string . actual-item) candidates for consult
         ...
         (selected (consult--read
                    ...
                    :sort nil             ;; Crucial: Do not re-sort the combined list
                    :require-match t
                    :category 'kill-ring
                    :lookup #'consult--lookup-cdr
                    :history 'consult--yank-history)))
    (when selected
      ;; 1. Copy selected item to the system clipboard (wl-copy)
      (with-temp-buffer
        (insert selected)
        ;; Replace with 'xclip' or similar if not on Wayland/wl-clipboard
        (call-process-region (point-min) (point-max) "wl-copy")) 
      
      ;; 2. Ensure the selected item is at the top of the Emacs kill-ring
